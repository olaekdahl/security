...
- **Macie is a paid service.** New accounts get a **30-day free trial** of sensitive-data
  discovery; after that you pay per GB of S3 objects inventoried and per GB scanned by
  classification jobs. Always run `08_cleanup.sh` when finished and consider disabling Macie
  (`aws macie2 disable-macie`) if you won't use it again, to avoid ongoing bucket-inventory charges.
- Verify the Macie session state at any time with:

  ```bash
  aws macie2 get-macie-session --region "$AWS_REGION" --query status --output text
  ```
