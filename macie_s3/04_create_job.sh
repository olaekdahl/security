...
JOB_ID="$(aws macie2 create-classification-job   --region "${AWS_REGION}"   --job-type ONE_TIME   --name "${JOB_NAME}"   --sampling-percentage "${SAMPLING_PERCENTAGE}"   --description "Macie demo job scanning ${S3_PREFIX} prefix in ${BUCKET_NAME}"   --s3-job-definition "{
